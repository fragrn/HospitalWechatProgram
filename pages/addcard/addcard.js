...
Page({
  ...
  visiting_cardVerifyFun:function({detail})
  {
      var viscard = detail.detail.value;
      if (viscard == undefined || viscard == "" || viscard == null) {
        $Message({
          content: "就诊卡号不能为空"
        });
      }else{
        this.visiting_card = viscard;
      }
  },
  hospitalVerifyFun : function({detail})
  {
    var hospital = detail.detail.value;
    if(hospital == undefined || hospital == "" || hospital == null)
    {
      $Message({
        content: "就诊医院不能为空"
      });
