...
Page({
    ...
    for(let i=0;i<cloud_Paths.length;i++) {
      wx.cloud.uploadFile({
        ...
        success: (res) => {
          ...
          db.collection('electricrecord').add({
  ...
  showimage: function () {
    ...
    db.collection('electricrecord').where({
      ...
      success: function (res) {
        ...
        that.setData({
          ...
          fileList.forEach(fileID => {
            const fullFileID = "cloud://project-4gz9hqn3031f5a54.7072-project-4gz9hqn3031f5a54-1316546914/" + fileID;//NeedToChangeWhenCloudEnvironmentChanges
  ...
  deleteImage: function (imageUrl) {
    ...
    const fileID='cloud://project-4gz9hqn3031f5a54.7072-project-4gz9hqn3031f5a54-1316546914/'+fileid; //NeedToChangeWhenCloudEnvironmentChanges
        ...
        db.collection('electricrecord').where({
          ...
          success: function (result) {
            if (result.data.length > 0) {
              ...
              // 根据 _id 删除记录
              db.collection('electricrecord').doc(docId).remove({
                success: function (res) {
